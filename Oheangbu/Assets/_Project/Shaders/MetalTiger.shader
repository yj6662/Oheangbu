Shader "Oheangbu/MetalTiger"
{
 Properties{
  _BaseMap("Silver color",2D)="white"{}
  _NormalMap("Silver normal",2D)="bump"{}
 ...
 SubShader{
  ...
  Pass{
   ...
   half4 frag(V i):SV_Target{
    ...
    float3 view=normalize(GetWorldSpaceViewDir(i.world));float3 halfDir=normalize(view+l.direction);
    float spec=pow(saturate(dot(n,halfDir)),48)*saturate(dot(n,l.direction))*l.shadowAttenuation;
    color+=float3(.65,.72,.78)*spec*.65*l.color+float3(.65,.75,.82)*edge*.28;
