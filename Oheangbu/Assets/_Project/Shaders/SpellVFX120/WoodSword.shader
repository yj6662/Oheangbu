Shader "Oheangbu/VFX120/WoodSword"
{
    Properties
    {
        _BaseMap("Source colour and leaf cutout",2D)="white"{}
        _Assemble("Bamboo joints locked",Range(0,1))=1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _BumpScale, _TintStrength, _Saturation, _AlphaClip, _Cutoff, _Visibility, _GroundY, _Formation, _Assemble;
            CBUFFER_END
            struct Attributes { float4 positionOS:POSITION; float3 normalOS:NORMAL; float4 tangentOS:TANGENT; float2 uv:TEXCOORD0; float2 segment:TEXCOORD1; };
            ...
            {
                Varyings o;
                float locked=smoothstep(0,1,saturate(_Assemble*1.35-v.segment.x*.07));
                v.positionOS.x+=(1-locked)*.055*(fmod(v.segment.x,2)<.5?-1:1);
                v.positionOS.y-=(1-locked)*.035;
        ...
        Pass
        {
            ...
            struct ShadowAttributes { float4 positionOS:POSITION; float3 normalOS:NORMAL; float2 uv:TEXCOORD0; float2 segment:TEXCOORD1; };
