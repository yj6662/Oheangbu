Shader "Oheangbu/VFX120/BloomLeaf"
{
    Properties
    {
        _BaseMap("Source colour and leaf cutout",2D)="white"{}
        _PatternMap("KTP lotus",2D)="black"{}
        _PatternStrength("Lotus pigment",Range(0,1))=0
        _PatternTint("Lotus pigment colour",Color)=(.69,.64,.40,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                float4 _BaseMap_ST, _BaseColor, _Tint, _PatternTint;
                float _BumpScale, _TintStrength, _Saturation, _AlphaClip, _Cutoff, _Visibility, _GroundY, _Formation, _PatternStrength;
            CBUFFER_END
            ...
            half4 Frag(Varyings i, FRONT_FACE_TYPE face:FRONT_FACE_SEMANTIC):SV_Target
            {
                ...
                half2 patternUV=(i.uv-_BaseMap_ST.zw)/_BaseMap_ST.xy;
                half lotusMask=SAMPLE_TEXTURE2D(_PatternMap,sampler_PatternMap,patternUV).a;
                albedo=lerp(albedo,_PatternTint.rgb,lotusMask*_PatternStrength);
