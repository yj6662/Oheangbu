Shader "Oheangbu/VFX120/VineSurface"
{
    Properties
    {
        ...
        _Spread("Vine front",Range(0,1))=1
        _Dry("Dry pigment",Range(0,1))=0
        _Age("Time",Float)=0
        _PatternInk("KTP mask",Float)=0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _BumpScale, _TintStrength, _Saturation, _AlphaClip, _Cutoff, _Visibility, _GroundY, _Formation, _Spread, _Dry, _Age, _PatternInk;
            CBUFFER_END
            struct Attributes { float4 positionOS:POSITION; float3 normalOS:NORMAL; float4 tangentOS:TANGENT; float2 uv:TEXCOORD0; float2 stage:TEXCOORD1; };
            struct Varyings { float4 positionCS:SV_POSITION; float3 positionWS:TEXCOORD0; float3 normalWS:TEXCOORD1; float4 tangentWS:TEXCOORD2; float2 uv:TEXCOORD3; float fog:TEXCOORD4; float localZ:TEXCOORD5; float progress:TEXCOORD6; };
            Varyings Vert(Attributes v)
            {
                Varyings o;
                v.positionOS.y+=sin(v.positionOS.x*5+v.positionOS.z*3+_Age*1.4)*.006*v.stage.y*(1-_Dry);
                ...
                o.uv=TRANSFORM_TEX(v.uv,_BaseMap); o.fog=ComputeFogFactor(p.positionCS.z); o.localZ=v.positionOS.z; o.progress=v.stage.x; return o;
            }
            half4 Frag(Varyings i, FRONT_FACE_TYPE face:FRONT_FACE_SEMANTIC):SV_Target
            {
                ...
                clip(_Spread-.001);
                clip(_Spread-i.progress-tooth*.014);
                if(_Formation<.9999)
                {
                    clip(_Formation-.001);
                ...
                if(_PatternInk>.5){clip(source.a-.33);albedo=_Tint.rgb*.7;}
                ...
                albedo=lerp(albedo,half3(.16,.12,.065),_Dry*.72);
