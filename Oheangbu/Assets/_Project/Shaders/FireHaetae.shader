Shader "Oheangbu/FireHaetae"
{
 Properties{
  _EmberMask("Ember mask",2D)="black"{}
  _BaseMap("Charcoal color",2D)="white"{}
  _NormalMap("Charcoal normal",2D)="bump"{}
 ...
 SubShader{
  ...
  TEXTURE2D(_EmberMask);SAMPLER(sampler_EmberMask);TEXTURE2D(_BaseMap);SAMPLER(sampler_BaseMap);TEXTURE2D(_NormalMap);SAMPLER(sampler_NormalMap);
  ...
  Pass{
   ...
   half4 frag(V i):SV_Target{
    ...
    float mask=SAMPLE_TEXTURE2D(_EmberMask,sampler_EmberMask,i.uv).r;
