...
{
 [InitializeOnLoad] public static class KtpAreaWideAudit
 {
  const string Key="KtpAreaWideAudit";const BindingFlags Hidden=BindingFlags.Instance|BindingFlags.NonPublic;
  [Serializable] class Report {public string status;public List<string> checks=new List<string>(),errors=new List<string>();}
  static KtpAreaWideAudit(){EditorApplication.update+=Tick;}
  ...
  static void Tick()
  {
   ...
   File.WriteAllText(Path.Combine(Vfx120Editor.Output,"AreaWide","audit.json"),JsonUtility.ToJson(r,true));EditorApplication.isPlaying=false;
  }
  ...
  static void Run(Report r)
  {
   ...
   foreach(string glyph in new[]{"고","노","소","모","오"})
   {
    ...
    foreach(int fps in new[]{30,60,120})foreach(float speed in new[]{1f,.2f})
    {
     ...
     if(glyph=="오")Need(r,maxPeak-minPeak>.04f&&maxPeak<=2.26f&&minPeak>=1.94f,"water crest changes without root scaling "+fps+"/"+speed);
   ...
   var priorBook=AssetDatabase.LoadAssetAtPath<SpellBookSO>(KtpAreaWideBuild.Folder+"/BaselineSpellBook.asset");
