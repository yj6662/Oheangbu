...
{
 public static class StoneDokkaebiBuild
 {
  public const string Folder="Assets/_Project/Art/SpellVFX120/StoneDokkaebi";
  public static string Output=>Path.Combine(Vfx120Editor.Output,"StoneDokkaebi");
  ...
  {
   string path=Folder+"/"+name+".mat";var shader=Shader.Find("Oheangbu/StoneDokkaebi");if(shader==null||ShaderUtil.ShaderHasError(shader))throw new Exception("Stone dokkaebi shader compile failure");
   ...
   if(bark){mat.SetTexture("_BaseMap",AssetDatabase.LoadAssetAtPath<Texture2D>(Folder+"/Textures/T_StoneDokkaebi_BaseColor.png"));mat.SetTexture("_NormalMap",AssetDatabase.LoadAssetAtPath<Texture2D>(Folder+"/Textures/T_StoneDokkaebi_Normal.png"));}EditorUtility.SetDirty(mat);AssetDatabase.SaveAssetIfDirty(mat);return mat;
  }
  public static string Build()
  {
   if(EditorApplication.isPlayingOrWillChangePlaymode)throw new Exception("Stopped editor required");
   var p=Profile;if(!File.Exists(Folder+"/Baseline_Mom.asset"))AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath(p),Folder+"/Baseline_Mom.asset");
   var importer=(ModelImporter)AssetImporter.GetAtPath(Folder+"/SM_StoneDokkaebi.fbx");importer.isReadable=true;importer.importAnimation=false;importer.animationType=ModelImporterAnimationType.None;importer.importNormals=ModelImporterNormals.Calculate;importer.normalSmoothingAngle=75;importer.materialImportMode=ModelImporterMaterialImportMode.None;importer.SaveAndReimport();
   foreach(string kind in new[]{"BaseColor","Normal"}){var t=(TextureImporter)AssetImporter.GetAtPath(Folder+"/Textures/T_StoneDokkaebi_"+kind+".png");t.textureType=kind=="Normal"?TextureImporterType.NormalMap:TextureImporterType.Default;t.maxTextureSize=2048;t.SaveAndReimport();}
   var body=Material("M_WeatheredGranite",Color.white,true);
   var source=AssetDatabase.LoadAssetAtPath<GameObject>(Folder+"/SM_StoneDokkaebi.fbx");var model=new GameObject("PF_StoneDokkaebi_Static");var imported=Object.Instantiate(source,model.transform);imported.name="StoneDokkaebi_Body";ulong tris=0;
   try
   {
    ...
    p.StoneDokkaebiPrefab=PrefabUtility.SaveAsPrefabAsset(model,Folder+"/PF_StoneDokkaebi_Static.prefab");
   ...
   var debris=new GameObject("StoneDokkaebiDissolve");try
   {
