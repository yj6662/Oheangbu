...
{
    [InitializeOnLoad] public static class KtpBasicSixAudit
    {
        const string Key="KtpBasicSixAudit";
        ...
        static KtpBasicSixAudit(){EditorApplication.update+=Tick;}
        ...
        static void Tick()
        {
            ...
            File.WriteAllText(Path.Combine(Vfx120Editor.Output,"BasicSix","contact_audit.json"),JsonUtility.ToJson(r,true));EditorApplication.isPlaying=false;
        }
        ...
        static void Run(Report r)
        {
            ...
            need(cp.SpellProfiles.Length==10,"Exactly ten selected spell contact overrides");need(cp.ForSpell('모')==null&&cp.ForSpell(default)==null,"Other and missing letters retain default contacts");
            foreach(char glyph in new[]{'가','나','사','마','아'})
            {
                ...
                hit(5,'모');need(contacts.Count==count+2&&Mathf.Approximately(last().transform.localScale.x,cp.ParryScale),"Other glyph uses unchanged scale");
