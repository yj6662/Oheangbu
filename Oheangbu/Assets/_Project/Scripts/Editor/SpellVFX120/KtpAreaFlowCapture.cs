...
namespace Oheangbu.EditorTools.SpellVFX120
{
    ...
    [InitializeOnLoad] public static class KtpAreaFlowCapture
    {
        const string Key="KtpAreaFlowCaptureState";
        ...
        static KtpAreaFlowCapture(){EditorApplication.update+=Tick;Application.logMessageReceived+=(m,s,t)=>{if(SessionState.GetBool(Key,false)&&report!=null&&(t==LogType.Error||t==LogType.Exception))report.errors.Add(m);};}
        public static string Start(string glyph)
        {
            if(EditorApplication.isPlayingOrWillChangePlaymode||!new[]{"고","노","소","오"}.Contains(glyph))throw new Exception("Stopped C2 and one selected glyph required");
        ...
        static void Setup()
        {
            ...
            folder=Path.Combine(Vfx120Editor.Output,"AreaFlow",report.glyph);Directory.CreateDirectory(folder);
        ...
        static void BeginClip()
        {
            ...
            row=new Row{variant=after?"after":"before",view=external?"external":"play",commitRatio=ratio,eventSource="Production area resolver and scheduled damage; diagnostic targets. Before uses saved AreaWide profiles and SpellBook; after uses current profiles and SpellBook.",status="RUNNING",origin=origin,target=target};
            ...
            var selected=after?entry.Profile:AssetDatabase.LoadAssetAtPath<Vfx120Profile>(Vfx120Editor.AssetRoot+"/AreaFlow/Baseline_"+report.glyph+".asset");
        ...
        static CastPlan PlanArea()
        {
            ...
            var book=AssetDatabase.LoadAssetAtPath<SpellBookSO>(row.variant=="before"?KtpAreaFlowBuild.Folder+"/BaselineSpellBook.asset":"Assets/_Project/Data/Configs/SpellBook_Proto.asset");
