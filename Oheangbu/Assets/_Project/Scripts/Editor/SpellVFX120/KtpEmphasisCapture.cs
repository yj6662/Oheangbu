...
namespace Oheangbu.EditorTools.SpellVFX120
{
    ...
    [InitializeOnLoad] public static class KtpEmphasisCapture
    {
        const string Key="KtpEmphasis4Capture";
        ...
        static KtpEmphasisCapture(){EditorApplication.update+=Tick;Application.logMessageReceived+=(m,s,t)=>{if(SessionState.GetBool(Key,false)&&report!=null&&(t==LogType.Error||t==LogType.Exception))report.errors.Add(m);};}
        public static string Start(string glyph)
        {
            if(EditorApplication.isPlayingOrWillChangePlaymode||!new[]{"가","나","거","너"}.Contains(glyph))throw new Exception("Stopped C2 and one selected glyph required");
        ...
        static void Tick()
        {
            ...
            try
            {
                ...
                {
                    impact=true;var element=profile.Family=="Fire"||profile.Glyph=="너"?Element.Fire:Element.Wood;
                    var attack=element==Element.Fire?Element.Metal:Element.Earth;
        ...
        static void Setup()
        {
            ...
            folder=Path.Combine(Vfx120Editor.Output,"Emphasis4",report.glyph+"_v6");Directory.CreateDirectory(folder);
        ...
        static void BeginClip()
        {
            ...
            var selected=after?entry.Profile:AssetDatabase.LoadAssetAtPath<Vfx120Profile>(Vfx120Editor.AssetRoot+"/KtpEmphasis/BaselineV5_"+report.glyph+".asset");
            if(selected==null)throw new Exception("V5 baseline missing");profile=Object.Instantiate(selected);
            ...
            effect.SetImpactClock(.65f);effect.Begin(origin,guard?null:enemy.transform,target+Vector3.up*1.1f,Color.white);
            start=Time.time;
            Element element=report.glyph=="나"||report.glyph=="너"?Element.Fire:Element.Wood;
            if(guard)
            {
                Invoke(wiring,"ResolveParry",new SpellCast(report.glyph[0],SpellKind.Parry,element,1,default,1));
                if(element==Element.Wood)Vfx120Effect.SelectBambooGuard(effect);else Vfx120Effect.SelectFireGuard(effect);
            ...
            else AddPending(report.glyph[0],element,5,Time.time+.65f);
