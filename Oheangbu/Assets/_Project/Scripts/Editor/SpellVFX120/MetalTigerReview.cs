...
{
 [InitializeOnLoad]public static class MetalTigerReview
 {
  ...
  static MetalTigerReview(){EditorApplication.update+=Tick;}
  static void Save(){Directory.CreateDirectory(MetalTigerBuild.Output);File.WriteAllText(Path.Combine(MetalTigerBuild.Output,"capture.json"),JsonUtility.ToJson(report,true));}
  public static string Start(bool externalOnly=false)
  {
   ...
   Report previous=externalOnly?JsonUtility.FromJson<Report>(File.ReadAllText(Path.Combine(MetalTigerBuild.Output,"capture.json"))):null;
   ...
   origin=Camera.main.transform.position;forward=Vector3.ProjectOnPlane(Camera.main.transform.forward,Vector3.up).normalized;view=externalOnly?1:0;report=new Report{status="RUNNING",mvid=typeof(MetalTigerVfx).Module.ModuleVersionId.ToString()};
   if(externalOnly)report.clips.Add(previous.clips.First(c=>c.view=="play"));Save();return "METAL_TIGER_CAPTURE_STARTED";
  }
  static void BeginClip(bool resumeChunk=false)
  {
   if(FixedWardReview.Memory()>=.85f)throw new Exception("Memory >=85%; next clip stopped");host=new GameObject("MetalTigerReview");fx=host.AddComponent<Vfx120Effect>();fx.Profile=MetalTigerBuild.Profile;fx.PreviewControlled=true;fx.Begin(origin,null,origin+forward*10,Color.white);
   if(!fx.MetalTiger.Grounded)throw new Exception("Placement ground missing or >15cm deviation");
   cameraGo=new GameObject("MetalTigerReviewCamera");camera=cameraGo.AddComponent<Camera>();camera.CopyFrom(Camera.main);camera.enabled=false;camera.targetTexture=null;camera.aspect=16f/9;camera.fieldOfView=65;camera.nearClipPlane=.08f;camera.GetUniversalAdditionalCameraData().renderPostProcessing=true;
   ...
   if(!resumeChunk){string name=view==0?"play":"external";clip=new Clip{view=name,folder=Path.Combine(MetalTigerBuild.Output,name),commitRatio=FixedWardReview.Memory(),footError=fx.MetalTiger.FootError};Directory.CreateDirectory(clip.folder);report.clips.Add(clip);frame=0;}Save();
  }
  static void Tick()
  {
   ...
   try
   {
    ...
    float age=frame/24f;fx.Sample(age);var center=fx.MetalTiger.Centre;
    ...
    clip.ended=fx.MetalTiger.Ended&&fx.MetalTiger.LiveParticles==0&&fx.MetalTiger.VisibleRenderers==0;if(!clip.ended)throw new Exception("End residue");Cleanup();clip=null;view++;Save();GC.Collect();EditorUtility.UnloadUnusedAssetsImmediate();if(view==1){report.status="PLAY_COMPLETE_EXTERNAL_PENDING";Save();report=null;}
  ...
  {
   if(EditorApplication.isPlayingOrWillChangePlaymode||report!=null)throw new Exception("Stopped editor required");var result=new AuditResult();var ground=GameObject.CreatePrimitive(PrimitiveType.Cube);ground.name="MetalTigerDiagnosticGround";ground.layer=6;ground.transform.position=new Vector3(7000,-.5f,7000);ground.transform.localScale=new Vector3(30,1,30);Physics.SyncTransforms();
   try
   {
    foreach(int fps in new[]{30,60,120})foreach(float rate in new[]{1f,.2f})
    {
     var go=new GameObject("TigerAudit");try{
      var effect=go.AddComponent<Vfx120Effect>();effect.Profile=MetalTigerBuild.Profile;effect.PreviewControlled=true;effect.Begin(new Vector3(7000,1.6f,7000),null,new Vector3(7000,1.6f,7010),Color.white);var tiger=effect.MetalTiger;
      if(!tiger.Grounded)throw new Exception("Ground placement");result.passed.Add("ground "+fps+"/"+rate);
      ...
      var original=tiger.Centre;var filters=go.GetComponentsInChildren<MeshFilter>().Where(x=>x.name.Contains("MetalTiger_Body")).ToArray();var originalVerts=filters.Select(x=>x.sharedMesh.vertices).ToArray();
      foreach(var foot in tiger.FeetWorld())
      {
      ...
      for(float t=0;t<4.7f;t+=rate/fps){effect.Sample(t);if(tiger.Centre!=original)throw new Exception("Drifting summon");}
      result.passed.Add("clock life "+fps+"/"+rate);if(!tiger.Ended||tiger.LiveParticles!=0||tiger.VisibleRenderers!=0)throw new Exception("End residue");result.passed.Add("clean end "+fps+"/"+rate);
      ...
      go.transform.position+=Vector3.right*4;effect.Sample(4.8f);if(Vector3.Distance(go.transform.Find("MetalTigerPresentation").position,original)>.001f)throw new Exception("Follows host");result.passed.Add("world anchoring "+fps+"/"+rate);
      foreach(var point in tiger.FeetWorld()){if(!ground.GetComponent<Collider>().Raycast(new Ray(point+Vector3.up,Vector3.down),out var hit,2))throw new Exception("Missing paw ground");float gap=Mathf.Abs(point.y-hit.point.y);result.maxFootGap=Mathf.Max(result.maxFootGap,gap);if(gap>.02f)throw new Exception("Hoof gap >2cm");}result.passed.Add("paw anchors <2cm "+fps+"/"+rate);
     }finally{Object.DestroyImmediate(go);}
    }
    result.status="PASS";
   }catch(Exception e){result.status="FAIL";result.errors.Add(e.ToString());}
   finally{Object.DestroyImmediate(ground);}
   File.WriteAllText(Path.Combine(MetalTigerBuild.Output,"audit.json"),JsonUtility.ToJson(result,true));return result.status+" "+result.passed.Count+" checks "+string.Join(";",result.errors);
  }
 }
}
