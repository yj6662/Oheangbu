...
{
 [InitializeOnLoad]public static class WaterTurtleReview
 {
  ...
  static WaterTurtleReview(){EditorApplication.update+=Tick;}
  static void Save(){Directory.CreateDirectory(WaterTurtleBuild.Output);File.WriteAllText(Path.Combine(WaterTurtleBuild.Output,"capture.json"),JsonUtility.ToJson(report,true));}
  public static string Start(bool externalOnly=false)
  {
   ...
   Report previous=externalOnly?JsonUtility.FromJson<Report>(File.ReadAllText(Path.Combine(WaterTurtleBuild.Output,"capture.json"))):null;
   ...
   origin=Camera.main.transform.position;forward=Vector3.ProjectOnPlane(Camera.main.transform.forward,Vector3.up).normalized;view=externalOnly?1:0;report=new Report{status="RUNNING",mvid=typeof(WaterTurtleVfx).Module.ModuleVersionId.ToString()};
   if(externalOnly)report.clips.Add(previous.clips.First(c=>c.view=="play"));Save();return "WATERTURTLE_CAPTURE_STARTED";
  }
  static void BeginClip(bool resumeChunk=false)
  {
   if(FixedWardReview.Memory()>=.85f)throw new Exception("Memory >=85%; next clip stopped");host=new GameObject("WaterTurtleReview");fx=host.AddComponent<Vfx120Effect>();fx.Profile=WaterTurtleBuild.Profile;fx.PreviewControlled=true;fx.Begin(origin,null,origin+forward*10,Color.white);
   if(!fx.WaterTurtle.Grounded)throw new Exception("Placement ground missing or >15cm deviation");
   cameraGo=new GameObject("WaterTurtleReviewCamera");camera=cameraGo.AddComponent<Camera>();camera.CopyFrom(Camera.main);camera.enabled=false;camera.targetTexture=null;camera.aspect=16f/9;camera.fieldOfView=65;camera.nearClipPlane=.08f;camera.GetUniversalAdditionalCameraData().renderPostProcessing=true;
   ...
   if(!resumeChunk){string name=view==0?"play":"external";clip=new Clip{view=name,folder=Path.Combine(WaterTurtleBuild.Output,name),commitRatio=FixedWardReview.Memory(),footError=fx.WaterTurtle.FootError};Directory.CreateDirectory(clip.folder);report.clips.Add(clip);frame=0;}Save();
  }
  static void Tick()
  {
   ...
   try
   {
    ...
    float age=frame/24f;fx.Sample(age);var center=fx.WaterTurtle.Centre;
    ...
    clip.ended=fx.WaterTurtle.Ended&&fx.WaterTurtle.LiveParticles==0&&fx.WaterTurtle.VisibleRenderers==0;if(!clip.ended)throw new Exception("End residue");Cleanup();clip=null;view++;Save();GC.Collect();EditorUtility.UnloadUnusedAssetsImmediate();if(view==1){report.status="PLAY_COMPLETE_EXTERNAL_PENDING";Save();report=null;}
  ...
  {
   if(EditorApplication.isPlayingOrWillChangePlaymode||report!=null)throw new Exception("Stopped editor required");var result=new AuditResult();var ground=GameObject.CreatePrimitive(PrimitiveType.Cube);ground.name="WaterTurtleDiagnosticGround";ground.layer=6;ground.transform.position=new Vector3(7000,-.5f,7000);ground.transform.localScale=new Vector3(30,1,30);Physics.SyncTransforms();
   try
   {
    foreach(int fps in new[]{30,60,120})foreach(float rate in new[]{1f,.2f})
    {
     var go=new GameObject("DokkaebiAudit");try{
      var effect=go.AddComponent<Vfx120Effect>();effect.Profile=WaterTurtleBuild.Profile;effect.PreviewControlled=true;effect.Begin(new Vector3(7000,1.6f,7000),null,new Vector3(7000,1.6f,7010),Color.white);var dokkaebi=effect.WaterTurtle;
      ...
      var original=dokkaebi.Centre;var filters=go.GetComponentsInChildren<MeshFilter>().Where(x=>x.name.Contains("WaterTurtle_Body")).ToArray();var originalVerts=filters.Select(x=>x.sharedMesh.vertices).ToArray();
      ...
      go.transform.position+=Vector3.right*4;effect.Sample(4.8f);if(Vector3.Distance(go.transform.Find("WaterTurtlePresentation").position,original)>.001f)throw new Exception("Follows host");result.passed.Add("world anchoring "+fps+"/"+rate);
   ...
   File.WriteAllText(Path.Combine(WaterTurtleBuild.Output,"audit.json"),JsonUtility.ToJson(result,true));return result.status+" "+result.passed.Count+" checks "+string.Join(";",result.errors);
  }
 }
}
