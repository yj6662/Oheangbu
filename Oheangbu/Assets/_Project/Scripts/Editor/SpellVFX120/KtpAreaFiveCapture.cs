...
namespace Oheangbu.EditorTools.SpellVFX120
{
    ...
    [InitializeOnLoad] public static class KtpAreaFiveCapture
    {
        const string Key="KtpAreaFiveCaptureState";
        ...
        {
            public string glyph,status,mvid,scene,limitation="C2 runtime area fixture using production SpellBook geometry and scheduled damage against three targets plus one outside target. Not handwriting input or enemy AI combat. Camera is frozen for before/after comparison. Capture is not a performance benchmark.";
        ...
        static KtpAreaFiveCapture(){EditorApplication.update+=Tick;Application.logMessageReceived+=(m,s,t)=>{if(SessionState.GetBool(Key,false)&&report!=null&&(t==LogType.Error||t==LogType.Exception))report.errors.Add(m);};}
        public static string Start(string glyph)
        {
            if(EditorApplication.isPlayingOrWillChangePlaymode||!new[]{"고","노","소","모","오"}.Contains(glyph))throw new Exception("Stopped C2 and one selected glyph required");
        ...
        static void Setup()
        {
            ...
            folder=Path.Combine(Vfx120Editor.Output,"AreaFive",report.glyph);Directory.CreateDirectory(folder);
        ...
        static void BeginClip()
        {
            ...
            row=new Row{variant=after?"after":"before",view=external?"external":"play",commitRatio=ratio,eventSource="Production area resolver and scheduled damage; diagnostic targets. Go baseline visuals replay the new staggered damage plan.",status="RUNNING",origin=origin,target=target};
            ...
            var selected=after?entry.Profile:AssetDatabase.LoadAssetAtPath<Vfx120Profile>(Vfx120Editor.AssetRoot+"/AreaFive/Baseline_"+report.glyph+".asset");
