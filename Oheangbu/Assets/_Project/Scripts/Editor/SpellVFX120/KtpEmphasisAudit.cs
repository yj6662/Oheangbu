...
{
    [InitializeOnLoad] public static class KtpEmphasisAudit
    {
        const string Key="KtpEmphasisAudit";
        ...
        static KtpEmphasisAudit(){EditorApplication.update+=Tick;}
        ...
        static void Tick()
        {
            ...
            File.WriteAllText(Path.Combine(Vfx120Editor.Output,"Emphasis4","contact_audit.json"),JsonUtility.ToJson(r,true));EditorApplication.isPlaying=false;
        }
        ...
        static void Run(Report r)
        {
            ...
            need(cp.SpellProfiles.Length==4,"Exactly four spell contact overrides");need(cp.ForSpell('마')==null&&cp.ForSpell(default)==null,"Other and missing letters retain default contacts");
            foreach(char glyph in new[]{'가','나'})
            {
                ...
                hit(5,'마');need(contacts.Count==count+2&&Mathf.Approximately(last().transform.localScale.x,cp.ParryScale),"Other glyph uses unchanged scale");
            ...
            foreach(char glyph in new[]{'거','너'})
            {
                Element element=glyph=='거'?Element.Wood:Element.Fire;
                Element success=element==Element.Wood?Element.Earth:Element.Metal;
                ...
                var fail=element==Element.Wood?Element.Fire:Element.Earth;count=contacts.Count;
