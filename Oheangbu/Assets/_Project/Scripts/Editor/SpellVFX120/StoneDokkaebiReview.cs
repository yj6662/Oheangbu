...
{
 [InitializeOnLoad]public static class StoneDokkaebiReview
 {
  ...
  static StoneDokkaebiReview(){EditorApplication.update+=Tick;}
  static void Save(){Directory.CreateDirectory(StoneDokkaebiBuild.Output);File.WriteAllText(Path.Combine(StoneDokkaebiBuild.Output,"capture.json"),JsonUtility.ToJson(report,true));}
  public static string Start(bool externalOnly=false)
  {
   ...
   Report previous=externalOnly?JsonUtility.FromJson<Report>(File.ReadAllText(Path.Combine(StoneDokkaebiBuild.Output,"capture.json"))):null;
   ...
   if(externalOnly)report.clips.Add(previous.clips.First(c=>c.view=="play"));Save();return "STONE_DOKKAEBI_CAPTURE_STARTED";
  }
  static void BeginClip(bool resumeChunk=false)
  {
   if(FixedWardReview.Memory()>=.85f)throw new Exception("Memory >=85%; next clip stopped");host=new GameObject("StoneDokkaebiReview");fx=host.AddComponent<Vfx120Effect>();fx.Profile=StoneDokkaebiBuild.Profile;fx.PreviewControlled=true;fx.Begin(origin,null,origin+forward*10,Color.white);
   if(!fx.StoneDokkaebi.Grounded)throw new Exception("Placement ground missing or >15cm deviation");
   cameraGo=new GameObject("StoneDokkaebiReviewCamera");camera=cameraGo.AddComponent<Camera>();camera.CopyFrom(Camera.main);camera.enabled=false;camera.targetTexture=null;camera.aspect=16f/9;camera.fieldOfView=65;camera.nearClipPlane=.08f;camera.GetUniversalAdditionalCameraData().renderPostProcessing=true;
   ...
   if(!resumeChunk){string name=view==0?"play":"external";clip=new Clip{view=name,folder=Path.Combine(StoneDokkaebiBuild.Output,name),commitRatio=FixedWardReview.Memory(),footError=fx.StoneDokkaebi.FootError};Directory.CreateDirectory(clip.folder);report.clips.Add(clip);frame=0;}Save();
  }
  static void Tick()
  {
  ...
  public static string Audit()
  {
   ...
   try
   {
    foreach(int fps in new[]{30,60,120})foreach(float rate in new[]{1f,.2f})
    {
     var go=new GameObject("DokkaebiAudit");try{
      var effect=go.AddComponent<Vfx120Effect>();effect.Profile=StoneDokkaebiBuild.Profile;effect.PreviewControlled=true;effect.Begin(new Vector3(7000,1.6f,7000),null,new Vector3(7000,1.6f,7010),Color.white);var dokkaebi=effect.StoneDokkaebi;
      ...
      var original=dokkaebi.Centre;var filters=go.GetComponentsInChildren<MeshFilter>().Where(x=>x.name.Contains("StoneDokkaebi_Body")).ToArray();var originalVerts=filters.Select(x=>x.sharedMesh.vertices).ToArray();
   ...
   File.WriteAllText(Path.Combine(StoneDokkaebiBuild.Output,"audit.json"),JsonUtility.ToJson(result,true));return result.status+" "+result.passed.Count+" checks "+string.Join(";",result.errors);
  }
 }
}
