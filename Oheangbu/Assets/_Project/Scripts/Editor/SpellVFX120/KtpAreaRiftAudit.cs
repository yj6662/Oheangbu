...
{
 [InitializeOnLoad] public static class KtpAreaRiftAudit
 {
  const string Key="KtpAreaRiftAudit";const BindingFlags Hidden=BindingFlags.Instance|BindingFlags.NonPublic;
  [Serializable] class Report {public string status;public List<string> checks=new List<string>(),errors=new List<string>();}
  static KtpAreaRiftAudit(){EditorApplication.update+=Tick;}
  ...
  static void Tick()
  {
   ...
   File.WriteAllText(Path.Combine(Vfx120Editor.Output,"AreaRift","audit.json"),JsonUtility.ToJson(r,true));EditorApplication.isPlaying=false;
  }
  ...
  static void Run(Report r)
  {
   ...
   foreach(string glyph in new[]{"고","노","소","모","오"})
   {
    ...
    foreach(int fps in new[]{30,60,120})foreach(float speed in new[]{1f,.2f})
    {
     ...
     if(glyph=="모")
     {
      ...
      Need(r,points.Count>0&&points.Max(x=>x.y)<2f&&points.Max(x=>x.y)>.2f,"rift debris remains low and grounded "+fps+"/"+speed);
      var cracks=fx.GetComponentsInChildren<LineRenderer>();fx.Sample(.5f);float early=cracks[0].GetPosition(24).z;fx.Sample(1.2f);float later=cracks[0].GetPosition(24).z;
      Need(r,cracks.Length==7&&later>early+3&&Mathf.Abs(later-(plan.Point.z+.8f*plan.Speed))<.01f,"fissure front advances on shared damage clock "+fps+"/"+speed);
     }
     if(glyph=="고"){fx.Sample(2.5f);Need(r,Mathf.Abs(fx.Life-3)<.001f&&fx.AreaRaised==17,"bamboo persists until near three second cutoff");fx.Sample(3.01f);Need(r,fx.GetComponentsInChildren<MeshRenderer>().All(x=>!x.enabled),"bamboo hidden after three seconds");}
   ...
   var priorBook=AssetDatabase.LoadAssetAtPath<SpellBookSO>(KtpAreaRiftBuild.Folder+"/BaselineSpellBook.asset");
   ...
   try{foreach(string g in new[]{"노","소"}){var profile=catalog.Entries.Single(x=>x.Glyph==g).Profile;var point=Vfx120LaunchPoint.Resolve(profile,cam,Vector3.zero);var viewport=cam.WorldToViewportPoint(point);Need(r,g=="노"?viewport.x>.6f&&viewport.y<.2f:viewport.y>1&&point.y>2,"launch framing "+g);}}
   finally{Object.DestroyImmediate(cameraGo);}
   var needle=catalog.Entries.Single(x=>x.Glyph=="소").Profile.NativeBodyPrefab;
