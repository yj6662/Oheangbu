...
namespace Oheangbu.App.SpellVFX120
{
 ...
 public sealed class WaterTurtleVfx:MonoBehaviour
 {
  ...
  public void Configure(Vfx120Profile p,Vector3 origin,Vector3 forward)
  {
   ...
   root=new GameObject("WaterTurtlePresentation");root.transform.SetParent(transform,false);root.transform.SetPositionAndRotation(Centre,rotation);
   model=Instantiate(p.WaterTurtlePrefab,root.transform);model.name="WaterTurtleStaticModel";
   ...
   if(anchors.Count!=4)throw new InvalidOperationException("4 ground support anchors required");feet=new Vector3[4];var offset=new float[4];
   ...
   seal.Configure(p.WaterTurtleSeal,p.Pigment,p.Ink,Vfx120TraditionalMotif.Role.Summon,settings);seal.Sample(1.2f);float diameter=0;
   ...
   sealGo.transform.localScale=Vector3.one*(4.2f/diameter);seal.Sample(0);
   var debris=Instantiate(p.WaterTurtleDebris,root.transform);systems=debris.GetComponentsInChildren<ParticleSystem>();foreach(var ps in systems){ps.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);ps.useAutoRandomSeed=false;ps.randomSeed=601;}
   Sample(0);
  }
