...
namespace Oheangbu.App.SpellVFX120
{
 ...
 public sealed class WoodDeerVfx:MonoBehaviour
 {
  ...
  public void Configure(Vfx120Profile p,Vector3 origin,Vector3 forward)
  {
   ...
   root=new GameObject("WoodDeerPresentation");root.transform.SetParent(transform,false);root.transform.SetPositionAndRotation(Centre,rotation);
   model=Instantiate(p.WoodDeerPrefab,root.transform);model.name="WoodDeerStaticModel";
   ...
   if(anchors.Count!=4)throw new InvalidOperationException("Four authored hoof anchors required");feet=new Vector3[4];var offset=new float[4];
   for(int i=0;i<4;i++){feet[i]=root.transform.InverseTransformPoint(anchors[i].position);Grounded&=Ground(anchors[i].position,out var g);offset[i]=g.y-anchors[i].position.y+.008f;if(Mathf.Abs(offset[i])>.15f)Grounded=false;}
   // Only the bottom 35cm conforms. The authored torso, face, antlers and leg lengths above it are untouched.
   foreach(var f in model.GetComponentsInChildren<MeshFilter>())
   {
    ...
    for(int k=0;k<v.Length;k++)
    {
     ...
     for(int j=0;j<4;j++){float d=Vector2.Distance(new Vector2(local.x,local.z),new Vector2(feet[j].x,feet[j].z));if(d<best){best=d;nearest=j;}}
   ...
   for(int i=0;i<4;i++){feet[i].y+=offset[i];var wp=root.transform.TransformPoint(feet[i]);if(Ground(wp,out var g))FootError=Mathf.Max(FootError,Mathf.Abs(wp.y-g.y));}
   // Short local roots emerge from the four hoof sites; they do not obscure legs or face.
   var rootMesh=Roots();owned.Add(rootMesh);var stalk=new GameObject("GroundRootStrands");stalk.transform.SetParent(root.transform,false);stalk.AddComponent<MeshFilter>().sharedMesh=rootMesh;
   var rr=stalk.AddComponent<MeshRenderer>();foreach(var r in model.GetComponentsInChildren<MeshRenderer>())if(r.name.Contains("Roots")){rr.sharedMaterial=r.sharedMaterial;break;}
   ...
   seal=sealGo.AddComponent<Vfx120TraditionalMotif>();var settings=Vfx120TraditionalMotif.Settings.DefaultFor(Vfx120TraditionalMotif.Role.Summon);settings.PreserveAuthored=true;settings.PreviewControlled=true;settings.HierarchyScaling=true;settings.PatternFocus=true;settings.FiniteWindow=true;settings.Lifetime=ReviewLife;settings.FadeSeconds=.8f;settings.Brightness=.65f;
   seal.Configure(p.WoodDeerSeal,p.Pigment,p.Ink,Vfx120TraditionalMotif.Role.Summon,settings);seal.Sample(.3f);Bounds sb=new Bounds(Centre,Vector3.zero);foreach(var r in seal.GetComponentsInChildren<Renderer>())sb.Encapsulate(r.bounds);sealGo.transform.localScale=Vector3.one*(2.25f/Mathf.Max(.01f,Mathf.Max(sb.size.x,sb.size.z)));seal.Sample(0);
   var debris=Instantiate(p.WoodDeerDebris,root.transform);systems=debris.GetComponentsInChildren<ParticleSystem>();foreach(var ps in systems){ps.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);ps.useAutoRandomSeed=false;ps.randomSeed=601;}
   Sample(0);
  }
  Mesh Roots()
  {
   var vertices=new List<Vector3>();var triangles=new List<int>();var uv=new List<Vector2>();
   for(int f=0;f<4;f++)for(int strand=0;strand<3;strand++)
   {
    int start=vertices.Count;float a=f*1.3f+strand*2.1f;
    for(int j=0;j<12;j++)
    {float t=j/11f;Vector3 c=feet[f]+new Vector3(Mathf.Cos(a+t)*.18f*(1-t),.28f*t,Mathf.Sin(a+t)*.18f*(1-t));for(int k=0;k<5;k++){float angle=k/5f*Mathf.PI*2;vertices.Add(c+new Vector3(Mathf.Cos(angle),0,Mathf.Sin(angle))*.009f);uv.Add(new Vector2(k/5f,t));if(j>0){int i=start+j*5+k,n=start+j*5+(k+1)%5;triangles.AddRange(new[]{i-5,n-5,i,n-5,n,i});}}}
   }
   var mesh=new Mesh{name="WoodDeerGroundRoots"};mesh.SetVertices(vertices);mesh.SetUVs(0,uv);mesh.SetTriangles(triangles,0);mesh.RecalculateNormals();mesh.RecalculateBounds();return mesh;
  }
  public void Sample(float age)
  {
   ...
   foreach(var ps in systems){float local=Age-3.8f;if(local>=0&&!Ended)ps.Simulate(local,false,true,false);else ps.Clear();}
  }
