...
namespace Oheangbu.App.SpellVFX120
{
 ...
 public sealed class StoneDokkaebiVfx:MonoBehaviour
 {
  ...
  public void Configure(Vfx120Profile p,Vector3 origin,Vector3 forward)
  {
   ...
   root=new GameObject("StoneDokkaebiPresentation");root.transform.SetParent(transform,false);root.transform.SetPositionAndRotation(Centre,rotation);
   model=Instantiate(p.StoneDokkaebiPrefab,root.transform);model.name="StoneDokkaebiStaticModel";
   ...
   if(anchors.Count!=2)throw new InvalidOperationException("Two planted foot anchors required");feet=new Vector3[2];var offset=new float[2];
   for(int i=0;i<2;i++){feet[i]=root.transform.InverseTransformPoint(anchors[i].position);Grounded&=Ground(anchors[i].position,out var g);offset[i]=g.y-anchors[i].position.y+.008f;if(Mathf.Abs(offset[i])>.15f)Grounded=false;}
   // Only the bottom 35cm conforms; torso, face and limbs above it stay static.
   foreach(var f in model.GetComponentsInChildren<MeshFilter>())
   {
    ...
    for(int k=0;k<v.Length;k++)
    {
     ...
     for(int j=0;j<2;j++){float d=Vector2.Distance(new Vector2(local.x,local.z),new Vector2(feet[j].x,feet[j].z));if(d<best){best=d;nearest=j;}}
   ...
   for(int i=0;i<2;i++){feet[i].y+=offset[i];var wp=root.transform.TransformPoint(feet[i]);if(Ground(wp,out var g))FootError=Mathf.Max(FootError,Mathf.Abs(wp.y-g.y));}
   ...
   seal.Configure(p.StoneDokkaebiSeal,p.Pigment,p.Ink,Vfx120TraditionalMotif.Role.Summon,settings);seal.Sample(1.2f);float diameter=0;
   ...
   sealGo.transform.localScale=Vector3.one*(3.4f/diameter);seal.Sample(0);
   var debris=Instantiate(p.StoneDokkaebiDebris,root.transform);systems=debris.GetComponentsInChildren<ParticleSystem>();foreach(var ps in systems){ps.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);ps.useAutoRandomSeed=false;ps.randomSeed=601;}
   Sample(0);
  }
  public void Sample(float age)
  {
   ...
   {
    bool forming=ps.name.StartsWith("Formation",StringComparison.Ordinal);float local=forming?Age:Age-3.8f;
