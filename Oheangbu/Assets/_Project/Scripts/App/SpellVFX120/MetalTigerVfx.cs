...
namespace Oheangbu.App.SpellVFX120
{
 ...
 public sealed class MetalTigerVfx:MonoBehaviour
 {
  ...
  public void Configure(Vfx120Profile p,Vector3 origin,Vector3 forward)
  {
   ...
   root=new GameObject("MetalTigerPresentation");root.transform.SetParent(transform,false);root.transform.SetPositionAndRotation(Centre,rotation);
   model=Instantiate(p.MetalTigerPrefab,root.transform);model.name="MetalTigerStaticModel";
   ...
   if(anchors.Count!=4)throw new InvalidOperationException("Four planted paw anchors required");feet=new Vector3[4];var offset=new float[4];
   ...
   seal.Configure(p.MetalTigerSeal,p.Pigment,p.Ink,Vfx120TraditionalMotif.Role.Summon,settings);seal.Sample(1.2f);float diameter=0;
   ...
   sealGo.transform.localScale=Vector3.one*(3.8f/diameter);seal.Sample(0);
   var debris=Instantiate(p.MetalTigerDebris,root.transform);systems=debris.GetComponentsInChildren<ParticleSystem>();foreach(var ps in systems){ps.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);ps.useAutoRandomSeed=false;ps.randomSeed=601;}
   Sample(0);
  }
