...
namespace Oheangbu.App.SpellVFX120
{
 ...
 public sealed class DokkaebiClubVfx:MonoBehaviour
 {
  ...
  public void Configure(Vfx120Profile p,Vector3 origin,Vector3 forward)
  {
   ...
   root=new GameObject("DokkaebiClubPresentation");root.transform.SetParent(transform,false);root.transform.SetPositionAndRotation(Centre,rotation);
   model=Instantiate(p.DokkaebiClubPrefab,root.transform);model.name="DokkaebiClubStaticModel";
   ...
   if(anchors.Count!=3)throw new InvalidOperationException("3 ground support anchors required");feet=new Vector3[3];var offset=new float[3];
   ...
   seal.Configure(p.DokkaebiClubSeal,p.Pigment,p.Ink,Vfx120TraditionalMotif.Role.Summon,settings);seal.Sample(1.2f);float diameter=0;
   ...
   var debris=Instantiate(p.DokkaebiClubDebris,root.transform);systems=debris.GetComponentsInChildren<ParticleSystem>();foreach(var ps in systems){ps.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);ps.useAutoRandomSeed=false;ps.randomSeed=601;}
   Sample(0);
  }
