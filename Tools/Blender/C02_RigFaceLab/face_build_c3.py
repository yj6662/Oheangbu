"""C3: additive skinned eyelids. Original C02 vertices/UV/weights are untouched.
...
selected_ids=json.loads((OUT/'face_surface_triangle_ids.json').read_text(encoding='utf-8'))['triangles']
tris=[tris[i] for i in selected_ids]
...
def build_lid(side,cx,zc):
    ...
    o['face_lab_role']='eyelid';o['face_lab_candidate']='C3';o['source_body_unchanged']=True
    return o
lids=[build_lid('Left',.02664062567,1.597148418),build_lid('Right',-.02749999985,1.597148418)]
folder=OUT/'C3';folder.mkdir(exist_ok=True)
manifest={'candidate':'C3','stage':'C','attempt':3,'method':'Additive skinned eyelid curtains fitted to raycast face surface. Painted eye geometry remains static beneath them. Sampling excludes disconnected hair islands and excessive front depth, using the 448-triangle actual eye-landmark connected face patch.',
...
for value,label in [(0,'open'),(.5,'half'),(1,'closed')]:
    ...
    ns['render']('C3_'+label+'_front',(0,-1,0))
for o in lids:o.data.shape_keys.key_blocks[1].value=0
rig.data.pose_position='POSE';bpy.context.view_layer.update()
bpy.ops.wm.save_as_mainfile(filepath=str(folder/'Face_C3.blend'))
print('FACE_C3_BUILT',json.dumps(manifest))
