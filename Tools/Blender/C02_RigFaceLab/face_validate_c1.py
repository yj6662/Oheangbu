"""C1 validation and true 30fps PNG/MP4 blink sequence, isolated background only."""
import bpy,json,math,hashlib,subprocess
import numpy as np
from pathlib import Path
from mathutils import Vector
from mathutils.bvhtree import BVHTree
ROOT=Path('C:/Users/yj666/Oheangbu');OUT=ROOT/'Art/PlayerPhase1/C02_RigFaceLab/Face';FOLDER=OUT/'C1'
...
for value,label in [(.25,'quarter'),(.75,'threequarter')]:
 values(value,value);ns['render']('C1_'+label+'_front',(0,-1,0))
values(1,1);ns['render']('C1_closed_threequarter',(1,-2,0));ns['render']('C1_closed_left',(1,0,0))
values(0,0)
report={'candidate':'C1','preservation':preservation,'checks':checks,'source_eyeballs':'NONE_SEPARATE','limits':'Occlusion of selected painted pupil landmarks only; no anatomical globe/lid contact claim. Surface-depth test uses the original fixed face shell.',
...
s=bpy.context.scene;s.render.engine='BLENDER_EEVEE_NEXT';s.eevee.taa_render_samples=8;s.render.fps=30;s.render.resolution_x=1280;s.render.resolution_y=720;s.render.resolution_percentage=100
...
video=FOLDER/'C1_Blink_30fps.mp4';command=[ffmpeg,'-y','-hide_banner','-loglevel','error','-framerate','30','-i',str(frames/'frame_%06d.png'),'-frames:v','60','-an','-c:v','libx264','-crf','18','-pix_fmt','yuv420p','-movflags','+faststart',str(video)]
...
print('C1_VALIDATION_COMPLETE',json.dumps(report))
