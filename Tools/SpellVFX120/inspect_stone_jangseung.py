...
R=Path('C:/Users/yj666/Oheangbu');O=R/'Art/SpellVFX120/StoneJangseung'
...
for loc in [(3,-4,5),(-3,-2,4),(1,3,4)]:
 bpy.ops.object.light_add(type='AREA',location=loc);l=bpy.context.object;l.data.energy=700;l.data.size=4;l.rotation_euler=(Vector((0,0,1.3))-l.location).to_track_quat('-Z','Y').to_euler()
bpy.ops.object.camera_add(location=(0,-5,2.2));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,1.3))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='ORTHO';cam.data.ortho_scale=4.5;scene.camera=cam;scene.render.resolution_x=1280;scene.render.resolution_y=720;scene.render.resolution_percentage=100;scene.render.filepath=str(O/'geometry_inspect.png');bpy.ops.render.render(write_still=True)
