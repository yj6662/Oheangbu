...
rows=[]
for glyph in '가나거너':
    r=json.loads((OUT/(glyph+'_v6')/'report.json').read_text(encoding='utf-8'))
...
(OUT/'rect_shield_scope.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
page=(ROOT/'Tools/SpellVFX120/rect_shield_review.html').read_text(encoding='utf-8')
(OUT.parent/'RECT_SHIELD_REVIEW.html').write_text(page.replace('__REPORTS__',json.dumps(rows,ensure_ascii=False)),encoding='utf-8')
