...
def wait(glyph):
    path=OUT/(glyph+'_v6')/'report.json'
...
if __name__=='__main__':
    wait('가')
    for glyph in '나거너':
        completed=OUT/(glyph+'_v6')/'report.json'
        if completed.exists() and json.loads(completed.read_text(encoding='utf-8'))['status']=='PASS_RUNTIME_FIXTURE_VISUAL_PENDING':continue
        print(call('RectShieldBuild',glyph),flush=True)
