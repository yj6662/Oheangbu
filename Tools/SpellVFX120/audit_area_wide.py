...
for method,path in [('AreaWideAudit',OUT/'AreaWide/audit.json'),('AreaWideRegression',OUT/'AreaWide/contact_audit.json'),('ContactAudit',OUT/'ktp_contact_play_audit.json')]:
    ...
    while time.monotonic()<deadline:
        if path.exists() and path.stat().st_mtime>previous:
            ...
            if r.get('status') not in ['RUNNING','PENDING']:
                ...
                if path.parent!=OUT/'AreaWide':shutil.copyfile(path,OUT/'AreaWide'/path.name)
                break
        time.sleep(1)
