...
def main() -> None:
    ...
    for source in direction['spells']:
        ...
        for phase in range(5):
            relative = f'Stills/{ident}_{phase}.png'
            present, check = image_state(output / relative)
            media.append({'phase': phase, 'path': relative if present else None,
    ...
    census = {
        'status': 'OFFLINE_GALLERY_CURRENT_FILES_ONLY_NOT_RUNTIME_PASS',
    ...
    template = HTML_TEMPLATE.replace('__DATA__', payload)
...
<h1>120개의 술식, 형태와 움직임으로 비교하기</h1>
<p class="intro">카드를 열면 같은 술식의 다섯 시점과 영상, 의도한 동작을 함께 볼 수 있습니다. 촬영본 안에는 글자나 검수 라벨을 얹지 않습니다.</p>
<div class="statistics" id="statistics"></div>
<p class="notice">배정된 100자 중 현재 게임 연결 경로는 15자입니다. 나머지 85자는 자산 검수 대상이며, 공백 20자는 효과가 배정되지 않은 시각 후보입니다. 이미지·영상 파일의 존재가 완성 판정을 뜻하지는 않습니다.</p>
</header>
...
<label class="sr-only" for="statusFilter">연결 상태</label><select id="statusFilter"><option value="all">모든 상태</option><option value="connected">기존 술식 연결 · 15</option><option value="asset">자산 검수 대상 · 85</option><option value="blank">미배정 시각 후보 · 20</option><option value="captured">촬영본 있음</option><option value="missing">미촬영</option></select>
...
<footer><span id="generated"></span><span>오프라인 검수 페이지 · <a href="build_manifest_validation.json">데이터 검사</a> · <a href="review_file_validation.json">현재 촬영 파일 목록</a></span></footer>
...
const labels={connected:'기존 술식 연결',asset:'자산 검수 대상',blank:'미배정 · 시각 후보'};
...
$('statistics').append(stat(120,'술식'),stat(census.capturedStills+'/600','이미지'),stat(census.presentVideos+'/120','영상'),stat(15,'게임 연결'),stat(20,'미배정'));
...
function showMedia(){
 ...
 if(mediaMode==='clip'){
  ...
  const sample=s.media[phase];if(sample.path){const img=node('img');img.src=sample.path;img.alt=s.glyph+' 검수 샘플 '+(phase+1);img.addEventListener('error',()=>{stage.replaceChildren(placeholder(s,true));$('mediaCaption').textContent='촬영 파일을 읽을 수 없습니다. 갤러리를 다시 생성해 주세요.';},{once:true});stage.append(img);
   const info=sample.inspection;$('mediaCaption').textContent=sample.path+' · '+info.width+' × '+info.height+(info.aspect16x9?' · 16:9':' · 화면 비율 확인 필요');
...
function openSpell(s){
 ...
 const source=$('sourceBox');source.replaceChildren(node('p','', '문양 원료 · '+s.patternName),node('p','source-path',s.pattern),node('p','', 'KoreanTraditionalPattern_Effect의 검수한 알파 문양을 새 연출에 사용합니다.'));
 ...
 $('detailNote').textContent=(s.status==='blank'?'이 글자는 의도적 공백입니다. 아래 연출은 시각 후보이며 전투 효과나 해금을 배정하지 않았습니다. ':s.status==='asset'?'현재 게임 이벤트 연결이 확인되지 않은 자산 검수 대상입니다. ':'기존 술식 연결 경로가 있는 항목입니다. 실제 시전·판정과 VFX 일치는 별도 검수합니다. ')+(s.sourceNote?'원본 비고: '+s.sourceNote+' ':'')+'위의 4단계 설명은 설계 의도이며 촬영 샘플의 시점을 임의로 확정하지 않습니다.';
