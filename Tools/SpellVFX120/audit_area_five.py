...
for method,path in [('AreaFiveAudit',OUT/'AreaFive/audit.json'),('AreaRegressionAudit',OUT/'AreaFive/contact_audit.json'),('ContactAudit',OUT/'ktp_contact_play_audit.json')]:
    ...
    while time.monotonic()<deadline:
        if path.exists() and path.stat().st_mtime>previous:
            ...
            if r.get('status') not in ['RUNNING','PENDING']:
                ...
                if path.parent!=OUT/'AreaFive':shutil.copyfile(path,OUT/'AreaFive'/path.name)
                break
        time.sleep(1)
